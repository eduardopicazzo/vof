...
vendor-onboarding-flow/
 ├── includes/
 │   │   ├── class-vof-core.php            # Core plugin logic (controller-like)
 │   │   ├── class-vof-dependencies.php    # Dependency management
 │   │   ├── class-vof-listing.php         # Listing creation/management
 │   │   ├── class-vof-subscription.php    # Subscription/payment logic
 │   │   ├── class-vof-form-handler.php    # Form submission logic
 │   │   ├── class-vof-stripe.php          # Stripe API integration
...
* **[ STEP-1:Listing-Details ] ->** The main purpose is to override current logic in order to allow for temporary listing creation (publishable upon successful checkout) for new/non-logged-in/non-subscribed (no membership active) users (vendors) that wish to post an ad (navigate to url with "post-an-ad" slug) 
    ...
    * **a wandering user/wandering vendor is considered:**
     ...
     * **[ CASE-3 ]->** "vendor-onboarding-plugin" plugin is activated AND a wandering user exists AND subscription/membership is not active AND navigates to url ending in: "/checkout/membership"
       * user should be redirected to stripe checkout section for subscription tier selection and payment.    

     * **[ CASE-4 ]->** "vendor-onboarding-plugin" plugin is activated AND user exists (logged in) AND subscription/membership is active AND navigates to url with slug "post-an-ad"
