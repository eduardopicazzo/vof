...
class VOF_Fulfillment_Handler {
     ...
     */
    public function vof_initiate_fulfillment($stripe_data, $new_sub = null, $subscription_id = null) { // $new_sub: has protected data, $subscription_id: not used nor needed
        try {
            global $wpdb;
            ...
            error_log('VOF Debug: INITIATING FULFILLMENT with $new_sub -> ' . print_r($new_sub, true));                 // $new_sub: has protected data
            error_log('VOF Debug: INITIATING FULFILLMENT with $subscription_id -> ' . print_r($subscription_id, true)); // $subscription_id: not used nor needed

            // change this read.. or pass the uuid
            $temp_user = $this->temp_user_meta->vof_get_temp_user_by_uuid(
                $stripe_data['metadata']['uuid'] // should be $stripe_data['uuid']
            );
